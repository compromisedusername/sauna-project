...
export class ReservationController {
	...
	public async getReservationsWithoutUser(req: Request, res: Response) {
		const result = await this.reservationService.getReservationsWithoutUser();
		return ResponseFactory.ok(res, result);
	}
	...
	public async addReservation(req: Request, res: Response): Promise<Response> {
		...
		const createdReservationId: number =
			await this.reservationService.addReservaton(reservationData);
	...
	public async updateReservation(
		...
		const reservationData: UpdateReservationRequest = {
			id: req.body.id,
			userId: req.body.userId,
			dateTo: req.body.dateTo,
			dateFrom: req.body.dateFrom,
			saunaId: req.body.saunaId,
			numberOfPeople: req.body.numberOfPeople,
		};

		const isUpdated: boolean =
			await this.reservationService.updateReservation(reservationData);
		if (isUpdated) {
